...
class Mutation(Enum):
    MULTIBIT_INVERSION_MUTATION = 1
    SINGLEBIT_INVERSION_MUTATION = 2


def mutation_gene(children, method, config):
    new_gene = np.copy(children)

    if (method == Mutation.MULTIBIT_INVERSION_MUTATION):
        new_gene = _bit_inversion_mutation_gene(children, config)
    if (method == Mutation.SINGLEBIT_INVERSION_MUTATION):
...
def _single_bit_mutation(children, config, bit=None):
    ...
    bitsize = children.shape[1]
    for g in new_gene:
        if random.random() < config.mutation_chance*10:
