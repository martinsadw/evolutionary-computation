...
def scatter_anim(data, duration, interval=20, name='test.gif'):
    frames_time = np.arange(0, duration, step=interval/1000)
    frames_x = []
    frames_y = []

    for (x, y, time) in data:
        frames_index = np.searchsorted(time, frames_time)
        frames_index = np.minimum(x.shape[0]-1, frames_index)
        frames_x.append(x[frames_index])
        frames_y.append(y[frames_index])

    frames_x = list(zip(*frames_x))
    frames_y = list(zip(*frames_y))
    frames = list(zip(frames_x, frames_y))
    ...
    for i in range(len(data)):
        scat = plt.scatter(frames_x[0][i], frames_y[0][i], s=10)
        scatters.append(scat)
    axes = fig.gca()

    def anim_func(frame, *fargs):
        (frames_x, frames_y) = frame
        for i in range(len(frames_x)):
            scatters[i].set_offsets(np.vstack((frames_x[i], frames_y[i])).T)

    anim = animation.FuncAnimation(fig, anim_func, frames=frames, interval=interval)
    # anim.save(name, writer='imagemagick', fps=30, extra_args=['convert'])
    anim.save('debug.mp4')
...
if __name__ == "__main__":
    ...
    (selected_materials, cost_value, best_fitness_array, population_fitness_array, perf_counter_array, process_time_array) = data

    used_data = population_fitness_array[0, 0]
    print(used_data.shape)

    time_span = 4
    data_time = np.arange(used_data.shape[0]) / used_data.shape[0]
    anim_data = [(used_data[:, :, 0], used_data[:, :, 1], data_time * time_span)]

    scatter_anim(anim_data, time_span, name='results/test.gif')

    ######################################
    # TEMP ###############################
    ######################################
    # (config.population_size, num_objectives)
    used_data = used_data[-1]

    fig = plt.figure()
    fig.suptitle('Objectives')
    plt.scatter(used_data[:, 0], used_data[:, 1])
    plt.show()
    ######################################
    ######################################
